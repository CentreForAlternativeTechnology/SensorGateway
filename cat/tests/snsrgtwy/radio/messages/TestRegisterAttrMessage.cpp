...
namespace cat { namespace snsrgtwy { namespace radio { namespace messages {
	...
	class TestRegisterAttrMessage : public RegisterAttrMessage {
		public:
			TestRegisterAttrMessage(char message[]) : RegisterAttrMessage(message) {}
			void register_attr() {
	...
	BOOST_AUTO_TEST_SUITE(RegisterAttrMessageTestSuite)
	
	BOOST_AUTO_TEST_CASE(should_register_attr) {
		char msg[] = {
		...
		EncryptedMessage::convert_to_encrypted_message(msg_encrypted, size, SHARED_KEY);
		
		TestRegisterAttrMessage m(msg_encrypted);
		m.register_attr();
